...
service = Service(executable_path="d:/pci/vendor/chromedriver-win64/chromedriver.exe")   
options = Options()
options.add_experimental_option("debuggerAddress", "127.0.0.1:9045")
...
for index, store in enumerate(stores):
    ...
    print(stores[index])
    print(bytes(vendor_name, 'utf-8').decode('unicode_escape').encode('latin1').decode('utf-8'))
    sheet.cell(row=start_row, column=1).value = vendor_name
    url = 'https://www.magazineluiza.com.br/lojista/' + bytes(vendor_name, 'utf-8').decode('unicode_escape').encode('latin1').decode('utf-8').replace(' ', '').replace('.', '')
    try:
        driver.get(url)
        print('done')
